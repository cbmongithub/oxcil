...
When we set out to build Oxcil, we knew that traditional scaling approaches wouldn't cut it. Large Language Models present unique challenges that require rethinking infrastructure from the ground up.
...
Stay tuned for more technical deep-dives into our infrastructure.
  ...
  },
  {
    ...
    title: "Introducing GPT-4 Turbo Support on Oxcil",
    excerpt:
      "We're excited to announce full support for OpenAI's GPT-4 Turbo model with 128K context window and improved performance.",
    content: `
# Introducing GPT-4 Turbo Support on Oxcil

Today we're thrilled to announce that GPT-4 Turbo is now available on the Oxcil platform. This represents a significant upgrade in capabilities for our users.
...
\`\`\`javascript
const response = await Oxcil.chat.completions.create({
  model: "gpt-4-turbo",
  messages: [
...
GPT-4 Turbo is available now for all Oxcil users. Check out our updated documentation for migration guides and best practices.
    `,
    author: {
      name: "Marcus Johnson",
...
Currently, these models support vision on Oxcil:
